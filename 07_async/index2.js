...
// - reslove() : 성공한 경우, 프로미스 사용시 then 메서드와 연결
...
/* function promise1(flag) {
  ...
  return new Promise(function (resolve, reject) {
    if (flag) {
      resolve(
        `promis 상태는 fulfilled(성공)!! then으로 연결됩니다. 이 때의 flag는  ${flag} 입니다.`
      );
    } else {
      reject(
        `promis 상태는 rejected!! catch으로 연결됩니다. 이 때의 flag는  ${flag} 입니다.`
      );
    }
  });
}
...
/* ---------------------------------------------------------------------------------- */
/* index.js에서 '콜백 함수'를 이용해 동기처리 한 것을 promise 이용해서 동기처리 해보자!*/
/* function goMart() {
...
pickDrink()
    ...
    .catch(nopay); //pickDrink 함수 안에서 reject 실행 시 catch로 연결
 */

/* -------------------------------------------------- */
/* Promise 체이닝 ( chaining) */
//ex. 함수를 이용해서 (4+3) *2 -1 =13 연산을 해보자!
//1. add(4, 3)
//2. mul(add(4, 3), 2)
