...
// - try 블록에서 에러가 있따면, try 블록 실행이 중단 -> catch 블록 코드 실행
/* 
try {
  //코드 실행 ing~
  //에러 발생시 catch로 이동
...
/* ------------------------------------------------------------------------- */
//index2.js에서 작성한 코드를 async/await 이용해서 변경하기
function goMart() {
...
async function exec() {
  goMart(); //1번 실행
  await pickDrink(); //2번 실행 -> 기다려
  pay(); //2번이 완료 되어야 3번 실행
